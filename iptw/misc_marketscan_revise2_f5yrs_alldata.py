...
def shell_for_ml_marketscan(cohort_dir_name, model, niter=50, min_patients=500, stats=True, more_para='',
    ...
    fo = open('revise_shell_{}_{}_marketscan.sh'.format(model, cohort_dir_name), 'w')  # 'a'

    fo.write('mkdir -p output_marketscan/revise/{}/{}/log\n'.format(cohort_dir_name, model))
    n_cmd = n_drug = 0
    for x in name_cnt:
        k, v = x
            ...
            for ctrl_type in ['random', 'atc']:
                for seed in range(0, niter):
                    cmd = "python main_revise.py --data_dir ../ipreprocess/output_marketscan/{}/ --treated_drug {} " \
                          "--controlled_drug {} --run_model {} --output_dir output_marketscan/revise/{}/{}/ --random_seed {} " \
                          "--drug_coding gpi --med_code_topk 200 {} {} " \
                          "2>&1 | tee output_marketscan/revise/{}/{}/log/{}_S{}D200C{}_{}.log\n".format(
...
def shell_for_ml_marketscan_selectcov(cohort_dir_name, model, niter=50, min_patients=500, stats=True, more_para='',
                                      selected=[]):
    cohort_size = pickle.load(
        open(r'../ipreprocess/output_marketscan/{}/cohorts_size.pkl'.format(cohort_dir_name), 'rb'))
    name_cnt = sorted(cohort_size.items(), key=lambda x: x[1], reverse=True)
    ...
    for x in name_cnt:
        ...
        if ((not selected) and ((v >= min_patients) or (k in added_drug))) or (selected and (drug in selected)):
            ...
            for ctrl_type in ['random', 'atc']:
                for seed in range(0, niter):
                    cmd = "python main_revise_selectcov.py --data_dir ../ipreprocess/output_marketscan/{}/ --treated_drug {} " \
...
def shell_for_ml_marketscan_5yrs_selectcovDAG(cohort_dir_name, model, niter=50, min_patients=500, stats=True, more_para='',
                                 folder='', appendix='', selected=[]):
    cohort_size = pickle.load(
        open(r'../ipreprocess/output_marketscan/{}/cohorts_size.pkl'.format(cohort_dir_name), 'rb'))
    name_cnt = sorted(cohort_size.items(), key=lambda x: x[1], reverse=True)
    ...
    fo = open('revise2_marketscan_f5yrs_selecteCov{}_shell_{}_{}_selected.sh'.format(appendix, model, cohort_dir_name), 'w')  # 'a'
    ...
    for x in name_cnt:
        ...
        if ((not selected) and ((v >= min_patients) or (k in added_drug))) or (selected and (drug in selected)):
            ...
            for ctrl_type in ['random', 'atc']:
                for seed in range(0, niter):
                    cmd = "python main_revise2_selectcov_causal_discover.py --data_dir ../ipreprocess/output_marketscan/{}/ --treated_drug {} " \
...
def results_model_selection_for_ml(cohort_dir_name, model, drug_name, niter=50, figdump=True):
    cohort_size = pickle.load(
        open(r'../ipreprocess/output_marketscan/{}/cohorts_size.pkl'.format(cohort_dir_name), 'rb'))
    name_cnt = sorted(cohort_size.items(), key=lambda x: x[1], reverse=True)
    ...
    for drug in tqdm(drug_list):
        ...
        if figdump:
            for t in ['random', 'atc', 'all']:
                ...
                boxplot = rdf[idx].boxplot(column=[x + 'all_n_unbalanced_feat_iptw' for x in pre_col], fontsize=15,
                                           ax=ax1,
                                           showmeans=True)  # rot=25,
                ax1.axhline(y=5, color='r', linestyle='-')
                boxplot.set_title("{}-{}_S{}D200C{}_{}".format(drug, drug_name.get(drug), '0-19', t, model),
                                  fontsize=25)
                ...
                # fig = plt.figure(figsize=(20, 15))
                boxplot = rdf[idx].boxplot(column=[x + 'test_auc' for x in pre_col], fontsize=15, ax=ax2,
                                           showmeans=True)
                ...
                boxplot = rdf[idx].boxplot(column=[x + 'all_reduction_n_unbalance_percent' for x in pre_col],
                                           fontsize=15,
                ...
                boxplot = rdf[idx].boxplot(column=[x + 'test_reduction_n_unbalance_percent' for x in pre_col],
                                           fontsize=15,
...
def bar_plot_model_selection(cohort_dir_name, model, contrl_type='random', dump=True, colorful=True):
    ...
    idx_auc = dfall[c1] >= MIN_SUCCESS_RATE  # 0.1
    idx_smd = dfall[c2] >= MIN_SUCCESS_RATE  # 0.1
    idx = dfall[c3] >= MIN_SUCCESS_RATE  # 0.1
...
def arrow_plot_model_selection_unbalance_reduction(cohort_dir_name, model, contrl_type='random', dump=True,
    ...
    idx_auc = dfall['success_rate-' + c1 + '-all_n_unbalanced_feat_iptw'] >= MIN_SUCCESS_RATE  # 0.1
    idx_smd = dfall['success_rate-' + c2 + '-all_n_unbalanced_feat_iptw'] >= MIN_SUCCESS_RATE  # 0.1
    idx = dfall['success_rate-' + c30 + '-all_n_unbalanced_feat_iptw'] >= MIN_SUCCESS_RATE  # 0.1
...
if __name__ == '__main__':
    ...
    model = 'LR'  # 'LR'  # 'MLP'   #'LIGHTGBM' #'LSTM'
    exp_dir = r'output_marketscan/revise2_f5yrs_all'
