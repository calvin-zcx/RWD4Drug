...
def shell_for_ml_simulation(model, niter=10, start=0, more_para=''):
    ...
    fo = open('simulate_v2_shell_{}.sh'.format(model), 'w')  # 'a'

    fo.write('mkdir -p output/simulate_v2/{}/log\n'.format(model))
    r = 0
    for n in [3000, 3500, 4000, 4500, 5000]:  # [2500, 3000, 3500, 4000, 4500, 5000, 5500, 6000]: #[2000, 4000, 6000]:
        for covspec in ['correct', 'partial', 'incorrect']:
            for train_ratio in [0.8, 0.6]:
                drug = 'simun{}train{:.2f}{}'.format(n, train_ratio, covspec)
                for seed in range(start, niter):
                    cmd = "python main_revise_v2_testset_simulate.py --nsim {} --train_ratio {} " \
                          "--run_model {} --covspecs {} --output_dir output/simulate_v2/{}/ --random_seed {} {}" \
                          "2>&1 | tee output/simulate_v2/{}/log/{}_S{}D267_{}.log\n".format(
                        n, train_ratio,
                        model, covspec, model, seed, more_para,
...
def bar_plot_ahr_coverage(model, groundtruth, contrl_type='all', dump=True,  datapart='all', log=False, ):
    ...
    rects2 = ax.bar(ind + width, top_2, width, # yerr=[top_2 - top_2_ci[:, 0], top_2_ci[:, 1] - top_2], error_kw=error_kw,
                    color=colors[1], edgecolor=None)
    rects3 = ax.bar(ind + 2 * width, top_3, width, # yerr=[top_3 - top_3_ci[:, 0], top_3_ci[:, 1] - top_3], error_kw=error_kw,
...
def aHR_evaluation_table(model, groundtruth, contrl_type='all', dump=True, colorful=True, datapart='all', log=False, ):
    ...
    for drug, drugname in zip(drug_list, drug_name_list):
        ...
        for ith, c in enumerate([c1, c2, c3]):
            ...
            mse_ori = mean_squared_error(np.ones_like(idx) * groundtruth, np.array(rdf.loc[idx, c + '-{}_HR_ori'.format(datapart)])) # mean_squared_error(y_true, y_pred)
            mse_iptw = mean_squared_error(np.ones_like(idx) * groundtruth, np.array(rdf.loc[idx, c + '-{}_HR_IPTW'.format(datapart)])) # mean_squared_error(y_true, y_pred)
