...
class LSTMModel(torch.nn.Module):
    def __init__(self, diag_vocab_size, med_vocab_size, diag_embedding_size,
        ...
        self.hidden2out_ipw = torch.nn.Linear(hidden_size, 1, bias=False)  # outputs logits, not using sigmoid here for probability
    ...
    def med_encode(self, inputs):  # bs * Time * Dim
        ...
        inputs_emb = self.med_embedding(inputs.float())  # bs * Time * HiddenDim  # change this to graph convolution later
        ...
        original = torch.sum(inputs.float() * att_normalized.unsqueeze(-1), dim=1)  # (bs * Time * Dim) * (bs * Time) attention over time --> (bs * Dim) time attention weighted sum of baseline features
    ...
    def forward(self, confounder):
        ...
        original = torch.cat((diag_original, med_original, sexes.float().view(sexes.size(0),1), ages.float().view(ages.size(0),1)),dim=1)  # bs * (D1+D2+2)

        # IPW
        hidden = torch.cat((diag_hidden, med_hidden, sexes.float().view(sexes.size(0),1), ages.float().view(ages.size(0),1)),dim=1)  # bs * (hidden1+hidden2+2)
